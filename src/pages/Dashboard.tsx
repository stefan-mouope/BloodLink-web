...
const Dashboard = () => {
  ...
  const role: UserRole | undefined = user?.user_type as UserRole;

  const userName = useMemo(() => {
    if (!user) return "Utilisateur";
    if (user.user_type === "banque") return user.nom ?? "Banque";
    if (user.user_type === "donneur")
      return [user.prenom, user.nom].filter(Boolean).join(" ") || "Donneur";
    return "Utilisateur";
  }, [user]);
    ...
    queryFn: async () => {
      if (role === "banque") {
        ...
        return (r.data || []).map((it: any) => ({
          ...
          subtitle:
            it.message || it.description || it.region || it.localisation || "—",
          badge: it.niveau || it.priorite,
          badgeVariant:
            (it.niveau || it.priorite) === "urgent"
              ? "destructive"
              : "default",
